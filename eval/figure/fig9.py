...
y_limits = [(0, 150), (0, 25), (0, 330), (0, 150)]

# Define y-axis ticks for each subplot
y_ticks = [
    np.arange(0, 151, 30),  # For subplot 0
    np.arange(0, 26, 5),  # For subplot 1
    np.arange(0, 331, 50),  # For subplot 2
    np.arange(0, 151, 50)  # For subplot 3
]
...
for subplot_index, ax in enumerate(axs):
    ...
    ax.set_ylabel("$99^{th}$-percentile\nRead/Scan Latency\n($\mu$s)" if subplot_index == 0 else "")
    ...
    ax.tick_params(axis='x', direction='in', length=6, width=0, colors='k',
                    grid_color='k', grid_alpha=0.5)
...
for thread in [1, 2, 3]:
    for config in ["Baseline", "XRP", "SODE"]:
        legend_handles.append(bar_dict[(thread, config_map[config])][0])
...
plt.savefig(f"data/fig9.pdf", format="pdf", bbox_inches="tight", dpi=300)
