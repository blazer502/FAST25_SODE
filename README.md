# SODE, Selective On-Device Execution of Data-Dependent Read I/Os
...
Futher details on the design and implementation of SODE can be found in the following paper.
...
1. Two or more CPU nodes
    **Your system must have two nodes at least because one is used for applications, and the other is used for computational storage emulation**. Currently, SODE is tested on two nodes system (Node 1 for applications, Node 2 for computational storage emulation).
2. Emulating wimpy on-device cores
    Some cores used by on-device computations need CPU frequency scaling to emulate 
    whimpy on-device cores. **You must select four cores at least for evaluations of SODE, and change [disable_cpu_freq_scaling.sh](utils/disable_cpu_freq_scaling.sh) script**. Currently, CPU 17, 18, 19, and 20 are used for on-device compuations. Other CPU 12, 13, 14, 15, and 16 are used for storage emulations. **In other words, your system must have a CPU node for emulation with 9 cores**. For the detailed explanation, see [SODE paper, 4.2 Emulating Wimpy On-device Cores](etc/fast25-sode.pdf).
3. Large memory binded to the node for computational storage emulation
    At least, we recommand 192GB memory binded to the node for computational storage emulation because YCSB benchmark requires large disk space.
4. Other requirements
    We recommand to **turn off hyper-threading, processor C-states, and turbo boost**.
...
#### Build SODE kernel
First, compile and install custom kernel (This process will require some time to install dependencies and compile the kernel from scratch):
```
cd src/kernel/
...
Fourth, load SODE emulator with 5 core numbers. The default cores for storage I/Os is 13, 14, 15, and 16. Then, the emulator setup cores for computational emulation is 17, 18, 19, and 20. The rest core 12 is used for dispatching I/O requests:
```
...
* Figure 5(d): Average latency for varying I/O index dpeth with single thread
...
* Figure 8(a): Throughput of BPF-KV using a open-loop load generator
...
#### Optional Comment
We apologize for the inconvenience caused because the code name of our project at the time of development was `HRP`. In our implementation, the key word is HRP not SODE. For example, you can find `read_hrp` instead of `read_sode`.
