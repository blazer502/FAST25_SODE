...
struct resubmit_data {
    ...
    // read only by storage
	u64 xrp_partition_start_sector;
	struct inode_operations *i_op;
	void *queuedata;

	struct xrp_extent *extents;
	int num_extents;
    //char padding2[4];
...
struct nvmev_io_worker {
	...
	volatile unsigned int r_seq; /* io req head index */
	volatile unsigned int r_seq_end; /* io req tail index */
    volatile int r_count_goal;

	unsigned long long latest_nsecs;

	unsigned int id;
	struct task_struct *task_struct;
    struct task_struct *resubmit_task;
...
struct nvmev_dev {
    struct task_struct *completion_task[3];
