...
static void __nvmev_admin_xrp_event(int eid)
{
	__make_cq_entry(eid, NVME_SC_SUCCESS);
}

static int total_cpu_roundtrip = 0;
static int avg_cpu_roundtrip = 0;
static int curr_cpu_roundtrip = 2000;
static int worst_cpu_roundtrip = 4000;
static int num_cpu_roundtrip = 0;

int get_curr_roundtrip_latency(void)
{
    return curr_cpu_roundtrip;
}
...
static void __nvmev_proc_admin_req(int entry_id)
{
    ...
    case nvme_cmd_hrp:
        ...
        if (on_meta->prev_cpu_arival != 0) {
            num_cpu_roundtrip += 1;
            curr_cpu_roundtrip = (int)on_meta->cpu_arival - (int)on_meta->prev_cpu_arival;
            if (curr_cpu_roundtrip > worst_cpu_roundtrip) {
                worst_cpu_roundtrip = curr_cpu_roundtrip;
            }
            total_cpu_roundtrip += curr_cpu_roundtrip;
            avg_cpu_roundtrip = total_cpu_roundtrip / num_cpu_roundtrip;
        }

        //printk("%d, %d", curr_cpu_roundtrip, avg_cpu_roundtrip);
            
        kunmap_atomic(base);

        __nvmev_admin_xrp_event(entry_id);
        break;
