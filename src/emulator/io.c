        ...
        for (i = 0; i < NR_MAX_PARALLEL_IO; i++) {
            ...
            if (w->is_completed == true && w->is_copied == true &&
                ...
                */
                ...
                if (prev_free_seq != -1) {
                    worker->work_queue[prev_free_seq].next = i;
                }
                ...
                */
                ...
                w->prev = prev_io_seq;
                if (prev_io_seq != -1) {
                    worker->work_queue[prev_io_seq].next = i;
                }
...
static void nvmev_init_latency_emulation(struct nvmev_io_work *w, unsigned long long artificial_delay)
{
    ...
    w->nsecs_start = nsecs_start;
    w->nsecs_enqueue = local_clock(); // maybe debugging purpose
    w->nsecs_target = ret.nsecs_target;
}
...
static int nvmev_resubmit_worker(void *data)
{
    ...
    u64 total_resubmit_time = 0;
    u64 total_resubmit_logic_time = 0;
    int resubmit_cnt = 0;
    u64 time_beg, time_end;

    u64 min_ebpf = 10000000, min_logic = 1000000, min_rest = 10000000;
    u64 max_ebpf = 0, max_logic = 0, max_rest = 0;

    u64 max_descent_index = 0, min_descent_index = 0;

    int moving = MOVING_LEN;
    int record_idx = 0;
    u64 recording_time = 0;
    int i = 0;
    int avg_time = 0;
    int avg_case = 0;
    int id;
        ...
        while (curr != -1) {
			...
			if (w->nsecs_target <= curr_nsecs) {
                if (w->is_sode) {
                    ...
                    if (w->on_meta->xrp_ebpf_time != 0) {
                        xrp_ebpf_time = w->on_meta->xrp_ebpf_time;
                        w->on_meta->xrp_ebpf_time = 0;
                    }
