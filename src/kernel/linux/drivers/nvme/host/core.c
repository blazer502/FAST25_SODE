...
int xrp_count_tree(struct inode *inode)
{
    struct rb_root *root;
    struct rb_node *node;
    int i = 0;
...
void xrp_copy_tree(struct inode *inode, struct xrp_extent *buf)
{
    struct rb_root *root;
    struct rb_node *node;
    int i = 0;
    ...
    while (node) {
        struct xrp_extent *i_extent;
		i_extent = rb_entry(node, struct xrp_extent, rb_node);
        memcpy(&buf[i], i_extent, sizeof(struct xrp_extent));
		i++;
        node = rb_next(node);
    }
...
static volatile u64 *backup_size[48] = { 0, };
spinlock_t backup_lock[48];

struct resubmit_data *__alloc_resubmit_req(struct request *req, void *dev)
{
	struct resubmit_data *new;
    ...
    if (spin_trylock(&backup_lock[smp_processor_id()])) {
        if (data_backup[smp_processor_id()] != NULL) {
            if (backup_size[smp_processor_id()] != size) {
                free_pages_exact(data_backup[smp_processor_id()], backup_size[smp_processor_id()]);
                base = alloc_pages_exact(size, GFP_KERNEL);
            }
            else {
                base = (void *)data_backup[smp_processor_id()];
                data_backup[smp_processor_id()] = NULL;
                backup_size[smp_processor_id()] = 0;
            }
        }
        else {
            base = alloc_pages_exact(size, GFP_KERNEL);
        ...
        spin_unlock(&backup_lock[smp_processor_id()]);
    }
    else {
        base = alloc_pages_exact(size, GFP_KERNEL);
        if (is_parallel) {
	...
	if (!base) {
		printk("%s: Failed to allocate", __func__);
		return NULL;
	}
    ...
    new = (struct resubmit_data *)base;

    new->size = size;

	new->xrp_bpf_prog = req->bio->xrp_bpf_prog;
	new->queuedata = req->q->queuedata;

	new->xrp_partition_start_sector = req->bio->xrp_partition_start_sector;
	new->i_op = req->bio->xrp_inode->i_op;

	new->xrp_data_page = page_address(bio_page(req->bio));
	new->xrp_scratch_page = page_address(req->bio->xrp_scratch_page);

    new->is_parallel = is_parallel;

	new->num_extents = num_extents;
	new->extents = (struct xrp_extent *)((u64)base + sizeof(struct resubmit_data));

    xrp_copy_tree(req->bio->xrp_inode, new->extents);

    root = rcu_dereference(req->bio->xrp_inode->xrp_extent_root); 
    if (root != NULL) {
        node = rb_first(root);
        extent = rb_entry(node, struct xrp_extent, rb_node);
        new->xrp_extent_version = extent->version; 
    }
    else {
        new->xrp_extent_version = 0; 
    }
    rcu_read_unlock();
    ...
    */
    ...
    if (is_parallel) {
        for (i = 0; i < 4; i++) {
            new->subtask_scratch[i] = (u64 *)((uint64_t)scratch + PAGE_SIZE * i);
            if (!new->subtask_scratch[i]) {
                int j = 0;
                printk("%s: Failed to allocate subtask pages", __func__);

                for (j = 0; j < i; j++) {
                    free_pages_exact(new->subtask_scratch[j], PAGE_SIZE);
                }

                free_pages_exact(base, size);
                return NULL;
            }
...
{
    struct resubmit_data *ret = __alloc_resubmit_req(req, dev);
    return virt_to_phys(ret) + ((u64)ret & ~PAGE_MASK);
}
...
void free_resubmit_req(struct resubmit_data *data, struct request *req)
{
	...
	// Send data to user
	//memcpy(page_address(bio_page(req->bio)), data->xrp_data_page, 0x1000);
	//memcpy(page_address(req->bio->xrp_scratch_page), data->xrp_scratch_page, 0x1000);

    if (spin_trylock(&backup_lock[smp_processor_id()])) {
        if (data->is_parallel) {
            if (scratch_backup[smp_processor_id()] == NULL) {
                scratch_backup[smp_processor_id()] = (u64 *)data->subtask_scratch[0];
            }
            else {
                free_pages_exact((void *)data->subtask_scratch[0], PAGE_SIZE * 4);
            }
        }

        if (data_backup[smp_processor_id()] == NULL) {
            data_backup[smp_processor_id()] = (u64 *)data;
            backup_size[smp_processor_id()] = data->size;
        }
        else {
            free_pages_exact((void *)data, data->size);
        }
        spin_unlock(&backup_lock[smp_processor_id()]);
    }
    else {
