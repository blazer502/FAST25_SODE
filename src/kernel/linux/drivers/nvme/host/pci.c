...
bool xrp_check_tree(struct inode *inode, struct xrp_extent *copy, int num_extents) {
    struct rb_root *root;
    struct rb_node *node;
    int i = 0;
    ...
    node = rb_first(root);
    while (node) {
        struct xrp_extent *original;
		original = rb_entry(node, struct xrp_extent, rb_node);
        if (original->lblk != copy[i].lblk || original->len != copy[i].len ||
                original->version != copy[i].version) {
            ret = false;
            break;
        }
        i++;
        node = rb_next(node);
        if (num_extents <= i && node != NULL) {
            ret = false;
            break;
        }
    }
...
static inline void nvme_handle_cqe(struct nvme_queue *nvmeq, u16 idx)
{
	...
	if (!req->bio || (!req->bio->hrp_enabled && !req->bio->xrp_enabled)) {
		...
		//struct resubmit_data *data = (struct resubmit_data *)(nvme_req(req)->cmd->rw.rsvd2);
        int i = 0;
        struct wt_ebpf_scratch *target, *scratch;
		...
		if (data->status > 0) {
            ...
            //if (!xrp_check_tree(req->bio->xrp_inode, data->extents, data->num_extents)) {
            if (data->xrp_extent_version != version) {
                u64 first_slba = data->first_slba;
				free_resubmit_req(data, req);
                req->xrp_command->rw.slba = first_slba;
                phys = alloc_resubmit_req(req, nvmeq->dev);
                if (phys == 0) {
                    if (!nvme_try_complete_req(req, cqe->status, cqe->result))
                        nvme_pci_complete_rq(req);
                    return;
                }
                req->xrp_command->rw.rsvd2 = (u64)phys;
		        nvme_submit_cmd(nvmeq, req->xrp_command, true);
				return;
            }
		...
		if (ebpf_context.done) {
            ...
            //per_cpu(per_cpu_done, smp_processor_id()) += 1;
        ...
        //data->previous_complete_counter = per_cpu(per_cpu_done, smp_processor_id()) + 1;
        //per_cpu(per_cpu_done, smp_processor_id()) = 0;
        ...
        if (data->is_parallel) {
            // Copy
            for (i = 0; i < 4; i++) {
                scratch = (struct wt_ebpf_scratch *)data->xrp_scratch_page;
                target = (struct wt_ebpf_scratch *)data->subtask_scratch[i];
